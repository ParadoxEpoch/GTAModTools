...
:: GTA III Install Path (e.g: C:\Games\GTA Trilogy\GTA III - Definitive Edition). Not required if autoinstall = false.
SET installdir=
...
SET pakname=999-CustomModIII_P
...
echo                     ^|     ^| ^|     ^| ^|     ^|
echo                     ^|     ^| ^|     ^| ^|     ^|
echo                  ___^|_____^|_^|_____^|_^|_____^|___
echo                 ^|                             ^|
echo                 ^|_____________________________^|
echo.
echo     --------------------------------------------------------
echo             Paradox's GTA III Definitive Mod Builder
...
if not exist "%originaldata%\GTA3\data\miss2.exe" if not "%skipcompile%" == "true" goto errnocompiler
if not exist "%originaldata%\GTA3\Scripts\main.sc" if not "%skipcompile%" == "true" goto errnoscript
...
echo Compiling GTA 3 Game Scripts...
cd %originaldata%\GTA3\data
miss2.exe -liberty -f ..\Scripts\main.sc -close
...
del ordered_commands.txt variable_info.txt variable_usage.txt GTA3SaveInfo.dat
cd %~dp0
...
echo You have not set the path to your GTA III installation files. Please edit this script and add a valid path.
...
:errnocompiler
TITLE %windowTitle% Error!
echo ERROR: "staging\Gameface\Content\OriginalData\GTA3\data\miss2.exe" is missing!
...
:errnoscript
TITLE %windowTitle% Error!
echo ERROR: "staging\Gameface\Content\OriginalData\GTA3\Scripts\main.sc" is missing!
